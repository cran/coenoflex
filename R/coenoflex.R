coenoflex <- function(numgrd,numplt,numspc,grdtyp,grdlen,width,variab,grdprd,alphad,
         pdist,sdist,skew,aacorr,cmpasy,cmpphy,maxtot,noise,slack,autlin)
{
    ...
    rndspc <- function (numspc,numgrd,grdlen,alphad,width,variab,grdtyp,skew,aacorr)
    {
        maxabu <- rep(0,numspc)
    ...
    fixspc <- function (numspc,numgrd,grdlen,width,variab,grdtyp,skew,aacorr)
    {
        spcamp <- array(0,dim=c(numspc,numgrd,5))
        maxabu <- rep(0,numspc)
        numpts <- rep(0,numgrd)
    ...
    if (missing(numgrd)) numgrd <- readline(' enter the number of gradients                   [1-10] : ')
    if (numgrd == '?') numgrd <- hlpgrd()
    ...
    if (missing(grdtyp)) grdtyp <- rep(NA,numgrd)
    ...
    if (missing(grdlen)) grdlen <- as.numeric(rep(NA,numgrd))
    else if (length(grdlen) != numgrd) stop('You must specify a gradient length for each gradient')    

    if (missing(width))  width <- rep(NA,numgrd)
    else if (length(width) != numgrd) stop('You must specify a mean species amplitude for each gradient') 

    if (missing(variab)) variab <- rep(NA,numgrd)
    else if (length(variab) != numgrd) stop('You must specify species amplitude variability for each gradient') 

    if (missing(grdprd)) grdprd <- rep(NA,numgrd)
    else if (length(grdprd) != numgrd) stop('You must specify the gradient productivity for each gradient')

    if (missing(alphad)) alphad <- rep(NA,numgrd)
    ...
    for (i in 1:as.numeric(numgrd)) {
        ...
        if (is.na(grdlen[i])) {
            grdlen[i] <- readline(paste(' enter gradient length for gradient ',i,'       [10-1000] : '))
            if (grdlen[i] == '?') grdlen[i] <- hlpglt()
...
#            numplt <- nrow(plot$centrd)
    ...
    if (missing(aacorr)) {
        ...
        if (aacorr == '?') aacorr <- hlphcn()
        aacorr <- as.numeric(aacorr)
        if (aacorr < 0.0 || aacorr > 1.0) {
    ...
    if (missing(sdist)) {
        ...
        if (sdist == '?') sdist <- hlpsrf()
        if (sdist == 'r' || sdist == 'R') {
            spc <- rndspc(numspc,numgrd,grdlen,alphad,width,variab,grdtyp,skew,aacorr)
        }    
        else {
            spc <- fixspc(numspc,numgrd,grdlen,width,variab,grdtyp,skew,aacorr)
        }
    }
    else {
        if (sdist != 'r' && sdist != 'R' && sdist != 'G' && sdist != 'g')
            stop ('The species distribution must be R (random) or G (grid)')
        else if (sdist == 'r' || sdist == 'R') spc <- rndspc(numspc,numgrd,grdlen,alphad,width,variab,grdtyp,skew,aacorr)
        else spc <- fixspc(numspc,numgrd,grdlen,width,variab,grdtyp,skew,aacorr)
    }

    if (missing(cmpasy)) {
        cmpasy <- readline(' enter the competition asymmetry            [1.00-3.00] : ')
        if (cmpasy == '?') cmpasy <- hlpasy()
...
#   }
    ...
    params <- list(numplt=numplt,numspc=numspc,numgrd=numgrd,grdtyp=grdtyp,
                   grdlen=as.numeric(grdlen),width=as.numeric(width),
                   variab=as.numeric(variab),grdprd=as.numeric(grdprd),
                   alphad=as.numeric(alphad),
                   skew=skew,aacorr=aacorr,cmpasy=cmpasy,cmpphy=cmpphy,
...
{
    if (class(x) != 'coenoflex') stop('must pass an object of class coenoflex')

    if (which == 'plots' | which == 'all') {
        for (i in 1:(x$params$numgrd-1)) {
            for (j in (i+1):x$params$numgrd) {
                plot(x$site[,i],x$site[,j],asp=1,xlab=paste('Gradient',i),
                   ylab=paste('Gradient',j),main='Plot Locations')
        ...
        for (i in 1:(x$params$numgrd-1)) {
            for (j in (i+1):x$params$numgrd) {
                plot(x$spcamp[,i,3],x$spcamp[,j,3],asp=1,
                     xlab=paste('Gradient',i),ylab=paste('Gradient',j),main='Species Centroids')
                     lines(c(0,x$params$grdlen[i],x$params$grdlen[i],0,0),
                           c(0,0,x$params$grdlen[j],x$params$grdlen[j],0),col=2)
                readline('hit return to continue')
             }
         }
    }
    if (which == 'amplitude' | which == 'all') {
        for (i in 1:x$params$numgrd) {
            ...
            plot(c(0,x$params$grdlen[i]),c(0,max(x$maxabu)),
                 xlab=paste('Gradient',i),ylab='Abundance',type='n')
